			...
			<div class="col filter-left">
				<div class="tab">
					<div class="btn-group manage_ads_tab" role="group" aria-label="Basic example">
						...
						<a class="active paid-ads btn btn-clear" href="{{ route('agent.paid_ads') }}">Paid Ads <span class="counter badge badge-success ml-1">{{ $count_premium }}</span></a>
					</div>
				</div>
			</div>
		</div>
...
<div id="my_content" class="">
	<div class="post_shortcut text-center bg-white border rounded mb-3">
		<div class="text">
		Hi <strong>p-{{ $user->phone }}</strong>, you currently have <strong>{{ $premium->count() }}</strong> ads (Paid Ads)
		</div>
		<div>
	...
	<ul class="list_posts list-unstyled">
		@if ($premium->count()>0)
			@foreach ($premium as $key => $property)
