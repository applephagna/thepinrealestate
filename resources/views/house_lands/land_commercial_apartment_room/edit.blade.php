...
@push('css')
	...
	<link  href="{{asset('assets/css/post.css')}}" rel="stylesheet" type="text/css">
	{{-- <link rel="stylesheet" href="{{asset('css/upload.css')}}"> --}}
	<link href="{{asset('assets/ksupload/ksupload.css')}}" rel="stylesheet">
    ...
    <link rel="stylesheet" href="{{asset('assets/sweetalert2/sweetalert2.min.css')}}">	<link rel="stylesheet" href="{{asset('assets/css/custom.css')}}">
@endpush
	...
	<div class="container">
		<div class="post_form bg-white rounded border my_content">
			...
			<div class="post_form p-3">
				<div class="post_form">
					<form action="{{route('agent.post.update',$property->id)}}" class="form form-horizontal" method="post" accept-charset="utf-8" enctype="multipart/form-data" id="form_post">
						{{csrf_field()}}
						{{ method_field('PUT') }}
...
@push('js')
<script src="{{asset('assets/ksupload/ksupload.js')}}"></script>
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#chk_save_contact").change( function() {
			if ( $(this).is(":checked") ) {
				$("#is_active").val(1)
