			...
			<div class="my-container">
				<div class="row">
					...
					<div class="col-3 left-side">
						<div class="p-3 bg-white border rounded filter-box">
							<div class="filter_title">Refine your Results</div>
								<form class="form" id="ftr_left" method="get" action="{{ route('property.by.type',$search_category) }}">
											...
											<div class="filter-data scrollbar-light">
												<select name="ad_bedroom" class="form-control list_filter fter_d form-control-">
													...
													@foreach ($bedrooms as $room =>$slug)
														<option value="{{ $slug }}" {{ $slug==$bedroom ?'selected':''}}
														>{{ $room }} Bedroom</option>
													@endforeach
													<option value="more" {{ $bedroom=='more'?'selected':'' }}>More+</option>
												</select>
											</div>
										</div>
										{{-- Bathroom --}}
											...
											<div class="filter-data scrollbar-light">
												<select name="ad_bathroom" class="form-control list_filter fter_d form-control-">
													...
													@foreach ($bathrooms as $room => $slug)
														<option value="{{ $slug }}" {{ $slug==$bathroom ?'selected':''}}
														>{{ $room }} Bathroom</option>
													@endforeach
													<option value="more" {{ $bathroom=='more'?'selected':'' }}>More+</option>
												</select>
											</div>
										</div>
										{{-- Facing --}}
												...
												<div class="list_filter fter_d">
													@foreach ($facings as $key => $facing)
														<div class="custom-control custom-radio">
															<input {{ strtolower($facing) == $face?'checked':''}} type="radio" id="ad_facing-{{ strtolower($facing) }}" name="ad_facing" class="custom-control-input" value="{{ strtolower($facing) }}">
									...
									<div class="form-group filter-group ">
										...
										<div class="form-row">
											<div class="filter-data col">
												<input type="text" name="from_ad_price" value="{{ $from_ad_price }}" class="form-control form-control-" placeholder="From">
											</div>
											<div class="filter-data col">
												<input type="text" name="to_ad_price" value="{{ $to_ad_price }}" class="form-control form-control-" placeholder="To">
											</div>
									...
									<div class="form-group filter-group ">
										...
										<div class="form-row">
											<div class="filter-data col">
												<input type="text" name="from_ad_size" value="{{ $from_ad_size }}" class="form-control form-control-" placeholder="From">
											</div>
											<div class="filter-data col">
												<input type="text" name="to_ad_size" value="{{ $to_ad_size }}" class="form-control form-control-" placeholder="To">
											</div>
											<div class="filter-data col-3">
												<button class="btn btn-default btn-"><span class="icon icon-next"></span></button>
											</div>
					...
					<div class="col col-9 right-side">
						<div class="bar">
							<div class="left">
									<h2 class="title">{{ $property_by_categories->count() }} Result on {{ now() }} </h2>
							</div>
