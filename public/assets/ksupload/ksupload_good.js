...
(function ($) {
	$.fn.Upload = function (option = false) {
		...
		pages = {
			...
			load_field: function () {
				var images = listImage.data("images");
				if (images) {
					countImage.text(images.length + '/' + pages.files_limit);
				}
			...
			uploadImage: function (files, target = false, from = 'input') {
					...
					formData.append('property_id', target.parents("form").find('[name="property_id"]').val());
			...
			rotateImage: function (id, target = false) {
					...
					$.ajax({
							...
							success: function (response) {
									if (response.success) {
											if (target) {
													target.parent().find('.img-contain').attr("src", response.image);
